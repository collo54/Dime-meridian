import 'dart:async';
import 'dart:convert';
import 'dart:io';
import 'dart:typed_data';

import 'package:audioplayers/audioplayers.dart';
import 'package:cloud_functions/cloud_functions.dart';
import 'package:flutter/foundation.dart';
import 'package:path_provider/path_provider.dart';
import 'package:uuid/uuid.dart';
import 'package:web_socket_channel/web_socket_channel.dart';

class ElevenLabsAgentService {
  WebSocketChannel? _channel;
  final AudioPlayer _player = AudioPlayer();
  final Uuid _uuid = const Uuid();
  final ValueNotifier<bool> isAgentSpeaking = ValueNotifier(false);

  // Buffer for Raw PCM Data
  final List<int> _pcmBuffer = [];

  Timer? _hardTimeout;
  bool _sessionActive = false;
  bool _closing = false;

  // ElevenLabs Agents usually stream at 16kHz (16000)
  static const int _sampleRate = 16000;
  static const Duration _maxSessionDuration = Duration(seconds: 30);

  Future<void> startSession({required String contextText}) async {
    if (_sessionActive) return;

    try {
      debugPrint("🗣️ Fetching ElevenLabs signed URL...");
      final result = await FirebaseFunctions.instance
          .httpsCallable('getAgentSignedUrl')
          .call();

      final data = Map<String, dynamic>.from(result.data);
      final signedUrl = data['signedUrl'] as String;

      debugPrint("🔌 Connecting...");
      _sessionActive = true;
      _closing = false;
      _pcmBuffer.clear();

      _channel = WebSocketChannel.connect(Uri.parse(signedUrl));

      _channel!.stream.listen(
        _onMessage,
        onError: (e) {
          debugPrint("❌ WebSocket error: $e");
          disconnect();
        },
        onDone: () {
          debugPrint("🔌 WebSocket closed by server");
          // If the server closes the connection (normal end of turn),
          // play whatever audio we managed to buffer.
          if (_pcmBuffer.isNotEmpty) {
            _playBufferedAudio();
          }
          _sessionActive = false;
        },
      );

      _hardTimeout = Timer(_maxSessionDuration, () {
        debugPrint("⏹️ Agent hard timeout reached");
        disconnect();
      });

      // 1️⃣ Send Initiation (Required by Spec)
      // We DO NOT request MP3 here because the API spec doesn't support it.
      // We accept the default PCM stream.
      _sendJson({
        "type": "conversation_initiation_client_data",
        // Optional: You can still set voice settings here if needed
        // "conversation_config_override": { "tts": { "stability": 0.5 } }
      });

      // 2️⃣ Wait briefly for handshake
      await Future.delayed(const Duration(milliseconds: 500));

      // 3️⃣ Send Text Prompt
      // The agent interprets "user_message" as speech to respond to.
      _sendJson({
        "type": "user_message",
        "text": "Summarize this data in 2 sentences: $contextText",
      });
    } catch (e) {
      debugPrint("❌ Start Session Failed: $e");
      disconnect();
    }
  }

  void _onMessage(dynamic message) async {
    if (_closing) return;

    final Map<String, dynamic> data = jsonDecode(message);

    // The spec uses 'type' to identify messages
    final type = data['type'];

    switch (type) {
      case 'audio':
        // Spec: "audio_event" contains "audio_base_64"
        final chunk = data['audio_event']?['audio_base_64'];
        if (chunk != null) {
          _pcmBuffer.addAll(base64Decode(chunk));
        }
        break;

      case 'agent_response':
        // Spec: "agent_response_event" contains "agent_response"
        debugPrint(
          "🤖 Agent Text: ${data['agent_response_event']?['agent_response']}",
        );
        break;

      case 'ping':
        // Spec: Respond with 'pong' and 'event_id'
        _sendJson({
          "type": "pong",
          "event_id": data['ping_event']?['event_id'],
        });
        break;

      case 'interruption':
        // If we interrupted the agent, clear buffer
        _pcmBuffer.clear();
        break;
    }
  }

  Future<void> _playBufferedAudio() async {
    if (_pcmBuffer.isEmpty) return;

    try {
      final dir = await getTemporaryDirectory();
      final file = File('${dir.path}/agent_${_uuid.v4()}.wav');

      // 1. Add WAV Header to Raw PCM
      // This tricks Android into playing the raw data correctly
      final wavBytes = _createWavHeader(_pcmBuffer.length, _sampleRate);
      wavBytes.addAll(_pcmBuffer);

      await file.writeAsBytes(wavBytes, flush: true);

      debugPrint("🎵 Playing generated WAV (${file.lengthSync()} bytes)...");

      // Notify UI: START
      isAgentSpeaking.value = true;

      await _player.play(DeviceFileSource(file.path));

      // Wait for playback to finish
      await _player.onPlayerComplete.first;
    } catch (e) {
      debugPrint("❌ Audio Playback Error: $e");
    } finally {
      // Notify UI: STOP (Even if error occurs)
      isAgentSpeaking.value = false;
    }
  }

  /// Creates a standard WAV header for 16-bit Mono PCM audio
  List<int> _createWavHeader(int pcmLength, int sampleRate) {
    int channels = 1;
    int byteRate = sampleRate * channels * 2; // 16-bit = 2 bytes
    int totalDataLen = pcmLength + 36;

    var header = Uint8List(44);
    var view = ByteData.view(header.buffer);

    _writeString(view, 0, 'RIFF');
    view.setUint32(4, totalDataLen, Endian.little);
    _writeString(view, 8, 'WAVE');
    _writeString(view, 12, 'fmt ');
    view.setUint32(16, 16, Endian.little);
    view.setUint16(20, 1, Endian.little); // PCM
    view.setUint16(22, channels, Endian.little);
    view.setUint32(24, sampleRate, Endian.little);
    view.setUint32(28, byteRate, Endian.little);
    view.setUint16(32, channels * 2, Endian.little);
    view.setUint16(34, 16, Endian.little); // 16-bit
    _writeString(view, 36, 'data');
    view.setUint32(40, pcmLength, Endian.little);

    return header.toList();
  }

  void _writeString(ByteData view, int offset, String value) {
    for (int i = 0; i < value.length; i++) {
      view.setUint8(offset + i, value.codeUnitAt(i));
    }
  }

  void _sendJson(Map<String, dynamic> payload) {
    if (_channel == null || _closing) return;
    _channel!.sink.add(jsonEncode(payload));
  }

  void disconnect() {
    if (_closing) return;
    isAgentSpeaking.value = false; // Reset UI
    _closing = true;
    _hardTimeout?.cancel();
    _channel?.sink.close();
    _player.stop();
    _sessionActive = false;
    debugPrint("🛑 Agent Disconnected.");
  }
}

// ////////// Gemini before raw code dpcs ***************************************************************************************
// import 'dart:async';
// import 'dart:convert';
// import 'dart:io';
// import 'dart:typed_data'; // Added for Uint8List

// import 'package:audioplayers/audioplayers.dart';
// import 'package:cloud_functions/cloud_functions.dart';
// import 'package:flutter/foundation.dart';
// import 'package:path_provider/path_provider.dart';
// import 'package:uuid/uuid.dart';
// import 'package:web_socket_channel/web_socket_channel.dart';

// class ElevenLabsAgentService {
//   WebSocketChannel? _channel;
//   final AudioPlayer _player = AudioPlayer();
//   final Uuid _uuid = const Uuid();

//   // Buffer to hold all MP3 chunks
//   final List<int> _audioBuffer = [];

//   Timer? _hardTimeout;
//   bool _sessionActive = false;
//   bool _closing = false;

//   static const Duration _maxSessionDuration = Duration(seconds: 30);

//   Future<void> startSession({required String contextText}) async {
//     if (_sessionActive) return;

//     try {
//       debugPrint("🗣️ Fetching ElevenLabs signed URL...");
//       final result = await FirebaseFunctions.instance
//           .httpsCallable('getAgentSignedUrl')
//           .call();

//       final data = Map<String, dynamic>.from(result.data);
//       final signedUrl = data['signedUrl'] as String;

//       debugPrint("🔌 Connecting...");
//       _sessionActive = true;
//       _closing = false;
//       _audioBuffer.clear(); // Clear previous buffer

//       _channel = WebSocketChannel.connect(Uri.parse(signedUrl));

//       _channel!.stream.listen(
//         _onMessage,
//         onError: (e) {
//           debugPrint("❌ WebSocket error: $e");
//           disconnect();
//         },
//         onDone: () {
//           debugPrint("🔌 WebSocket closed");
//           // Don't disconnect here, we might still be playing audio
//         },
//       );

//       _hardTimeout = Timer(_maxSessionDuration, () {
//         debugPrint("⏹️ Agent hard timeout reached");
//         disconnect();
//       });

//       // 1️⃣ CORRECT CONFIGURATION FOR AGENTS API
//       _sendJson({"type": "conversation_initiation_client_data"});

//       // 2️⃣ Send Prompt
//       _sendJson({
//         "type": "user_message",
//         "text": "Summarize this financial data in under 60 words: $contextText",
//       });
//     } catch (e) {
//       debugPrint("❌ Start Session Failed: $e");
//       disconnect();
//     }
//   }

//   void _onMessage(dynamic message) async {
//     if (_closing) return;

//     // Decode JSON safely
//     final Map<String, dynamic> data = jsonDecode(message);
//     final type = data['type'];

//     switch (type) {
//       case 'audio':
//         // Collect chunks. Since we requested MP3, these are MP3 frames.
//         // Concatenating MP3 frames results in a valid MP3 file.
//         final chunk = data['audio_event']?['audio_base_64'];
//         if (chunk != null) {
//           _audioBuffer.addAll(base64Decode(chunk));
//         }
//         break;

//       case 'agent_response':
//         debugPrint(
//           "🤖 Agent: ${data['agent_response_event']?['agent_response']}",
//         );
//         break;

//       case 'generation_complete':
//         debugPrint("✅ Generation Complete. Playing Audio...");
//         // Stop the timeout since we successfully got data
//         _hardTimeout?.cancel();

//         // Play the full buffer, then disconnect
//         await _playBufferedAudio();
//         disconnect();
//         break;

//       case 'ping':
//         _sendJson({"type": "pong", "event_id": data['event_id']});
//         break;

//       case 'error':
//         debugPrint("❌ Error from Server: ${data['error']}");
//         disconnect();
//         break;
//     }
//   }

//   Future<void> _playBufferedAudio() async {
//     if (_audioBuffer.isEmpty) {
//       debugPrint("⚠️ No audio received.");
//       return;
//     }

//     try {
//       final dir = await getTemporaryDirectory();
//       final file = File('${dir.path}/agent_response_${_uuid.v4()}.mp3');

//       // Write the accumulated MP3 bytes to a file
//       await file.writeAsBytes(_audioBuffer, flush: true);

//       debugPrint("🎵 Playing ${_audioBuffer.length} bytes...");

//       // Play
//       await _player.play(DeviceFileSource(file.path));

//       // CRITICAL FIX: Wait for playback to finish before function returns
//       // Otherwise disconnect() runs immediately and cuts off the audio.
//       await _player.onPlayerComplete.first;

//       debugPrint("🎵 Playback finished.");
//     } catch (e) {
//       debugPrint("❌ Audio Playback Error: $e");
//     }
//   }

//   void _sendJson(Map<String, dynamic> payload) {
//     if (_channel == null || _closing) return;
//     _channel!.sink.add(jsonEncode(payload));
//   }

//   void disconnect() {
//     if (_closing) return;
//     _closing = true;
//     _hardTimeout?.cancel();
//     _channel?.sink.close();
//     _player.stop();
//     _sessionActive = false;
//     debugPrint("🛑 Agent Disconnected.");
//   }
// }

// /////  Gpt version 1 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// import 'dart:async';
// import 'dart:convert';
// import 'dart:io';

// import 'package:audioplayers/audioplayers.dart';
// import 'package:cloud_functions/cloud_functions.dart';
// import 'package:flutter/foundation.dart';
// import 'package:path_provider/path_provider.dart';
// import 'package:uuid/uuid.dart';
// import 'package:web_socket_channel/web_socket_channel.dart';

// /// Production-safe ElevenLabs Agent WebSocket service
// ///
// /// Lifecycle:
// /// startSession() → receive audio → generation_complete → disconnect()
// class ElevenLabsAgentService {
//   WebSocketChannel? _channel;
//   final AudioPlayer _player = AudioPlayer();
//   final Uuid _uuid = const Uuid();
//   final List<int> _audioBuffer = [];

//   Timer? _hardTimeout;
//   bool _sessionActive = false;
//   bool _closing = false;

//   /// ===== CONFIG =====
//   static const Duration _maxSessionDuration = Duration(seconds: 20);
//   static const Duration _audioCleanupDelay = Duration(seconds: 10);

//   /// ==================
//   /// START SESSION
//   /// ==================
//   Future<void> startSession({required String contextText}) async {
//     if (_sessionActive) {
//       debugPrint("⚠️ Agent session already active. Ignoring.");
//       return;
//     }

//     try {
//       debugPrint("🗣️ Fetching ElevenLabs signed URL...");

//       final result = await FirebaseFunctions.instance
//           .httpsCallable('getAgentSignedUrl')
//           .call();

//       final data = Map<String, dynamic>.from(result.data);
//       final signedUrl = data['signedUrl'] as String;

//       debugPrint("🔌 Connecting to ElevenLabs WebSocket...");
//       _sessionActive = true;
//       _closing = false;

//       _channel = WebSocketChannel.connect(Uri.parse(signedUrl));

//       // Listen for server events
//       _channel!.stream.listen(
//         _onMessage,
//         onError: (e) {
//           debugPrint("❌ WebSocket error: $e");
//           disconnect();
//         },
//         onDone: () {
//           debugPrint("🔌 WebSocket closed");
//           disconnect();
//         },
//       );

//       // SAFETY: Hard timeout (prevents runaway credits)
//       _hardTimeout = Timer(_maxSessionDuration, () {
//         debugPrint("⏹️ Agent hard timeout reached");
//         disconnect();
//       });

//       // 1️⃣ Conversation initiation (REQUIRED by ElevenLabs)
//       _sendJson({
//         "type": "conversation_initiation_client_data",
//         "audio_output_config": {"format": "mp3", "sample_rate": 44100},
//       });

//       // 2️⃣ Send user text
//       _sendJson({
//         "type": "user_message",
//         "text":
//             "Summarize the following financial analysis in under 60 words:\n\n$contextText",
//         "try_trigger_generation": true,
//       });
//     } catch (e) {
//       debugPrint("❌ Agent startSession failed: $e");
//       disconnect();
//     }
//   }

//   /// ==================
//   /// MESSAGE HANDLER
//   /// ==================
//   void _onMessage(dynamic message) async {
//     if (_closing) return;

//     final Map<String, dynamic> data = jsonDecode(message);

//     final type = data['type'];

//     switch (type) {
//       case 'audio':
//         // final audioBase64 = data['audio_event']?['audio_base_64'];
//         // if (audioBase64 != null) {
//         //   await _playAudioChunk(audioBase64);
//         // }
//         final chunk = data['audio_event']?['audio_base_64'];
//         if (chunk != null) {
//           _audioBuffer.addAll(base64Decode(chunk));
//         }
//         break;

//       case 'agent_response':
//         debugPrint(
//           "🤖 Agent text: ${data['agent_response_event']?['agent_response']}",
//         );
//         break;

//       case 'generation_complete':
//         debugPrint("✅ Agent finished speaking");
//         // disconnect();
//         await _playBufferedAudio();
//         disconnect();
//         break;

//       case 'ping':
//         // Respond to keepalive pings
//         _sendJson({"type": "pong", "event_id": data['event_id']});
//         break;

//       case 'error':
//         debugPrint("❌ Agent error: ${data['error']}");
//         disconnect();
//         break;

//       default:
//         // Ignore other event types safely
//         break;
//     }
//   }

//   Future<void> _playBufferedAudio() async {
//     final dir = await getTemporaryDirectory();
//     final file = File('${dir.path}/el_final_${_uuid.v4()}.mp3');

//     await file.writeAsBytes(_audioBuffer, flush: true);
//     _audioBuffer.clear();

//     await _player.play(DeviceFileSource(file.path));
//   }

//   /// ==================
//   /// SEND JSON SAFELY
//   /// ==================
//   void _sendJson(Map<String, dynamic> payload) {
//     if (_channel == null || _closing) return;
//     _channel!.sink.add(jsonEncode(payload));
//   }

//   /// ==================
//   /// DISCONNECT (CREDIT STOP)
//   /// ==================
//   void disconnect() {
//     if (_closing) return;
//     _closing = true;

//     debugPrint("🛑 Disconnecting ElevenLabs agent");

//     _hardTimeout?.cancel();
//     _hardTimeout = null;

//     _channel?.sink.close();
//     _channel = null;

//     _player.stop();

//     _sessionActive = false;
//   }
// }
